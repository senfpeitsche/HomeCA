...
public sealed class HetznerDnsConnector(IHttpClientFactory clients, ILogger<HetznerDnsConnector> logger) : IDnsConnector
{
    ...
    public async Task<ConnectorCheckResult> CheckAsync(ConnectorSettings settings, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
                return new ConnectorCheckResult(false, [], $"Hetzner Cloud returned {(int)response.StatusCode}.");
            try
            {
        ...
        {
            logger.LogWarning(ex, "Hetzner DNS connectivity check failed");
            return new ConnectorCheckResult(false, [], $"Could not reach Hetzner API: {ex.Message}");
        }
    }
    ...
    public async Task UpsertTxtRecordAsync(ConnectorSettings settings, string recordName, string value, CancellationToken cancellationToken)
    {
        ...
        {
            var client = CreateClient(settings);
            var zone = await FindZoneAsync(client, recordName, cancellationToken);
            var rrsetName = ToRelativeName(recordName, zone.Name);
            ...
            if (existing.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
                ...
                logger.LogInformation("Created TXT record {RecordName} in zone {Zone}", recordName, zone.Name);
                return;
            }
            ...
            logger.LogInformation("Updated TXT record {RecordName} in zone {Zone}", recordName, zone.Name);
        }
        catch (HttpRequestException ex)
        {
            logger.LogError(ex, "Failed to upsert TXT record {RecordName} via Hetzner DNS", recordName);
            throw;
        }
    }
    ...
    public async Task DeleteTxtRecordAsync(ConnectorSettings settings, string recordName, string value, CancellationToken cancellationToken)
    {
        try
        {
            ...
            logger.LogInformation("Deleted TXT record {RecordName} from zone {Zone}", recordName, zone.Name);
        }
        catch (HttpRequestException ex)
        {
            logger.LogError(ex, "Failed to delete TXT record {RecordName} via Hetzner DNS", recordName);
            throw;
        }
    }
    ...
    {
        if (!settings.Secrets.TryGetValue("apiToken", out var token) || string.IsNullOrWhiteSpace(token))
            throw new InvalidOperationException("Hetzner API token is required.");
    ...
    private static async Task<Zone> FindZoneAsync(HttpClient client, string recordName, CancellationToken ct)
    {
        ...
        return zones
            .Where(zone => recordName.Equals(zone.Name, StringComparison.OrdinalIgnoreCase) || recordName.EndsWith('.' + zone.Name, StringComparison.OrdinalIgnoreCase))
            .OrderByDescending(zone => zone.Name.Length)
            .FirstOrDefault() ?? throw new InvalidOperationException($"No Hetzner zone owns {recordName}.");
    }
    ...
    private sealed record ZoneList(IReadOnlyList<Zone> Zones);
    private sealed record Zone(long Id, string Name);
}
