...
public sealed class CrlService(HomeCaStorage storage, RevocationRegistry revocations)
{
    public async Task<string> GenerateAsync(CancellationToken cancellationToken)
    {
        var issuerPath = Path.Combine(storage.RootPath, "authorities", "tls-issuing", "tls-issuing-ca.pfx");
        if (!File.Exists(issuerPath)) throw new InvalidOperationException("Initialize certificate authorities first.");
        using var issuer = X509CertificateLoader.LoadPkcs12FromFile(issuerPath, null);
        ...
        var path = Path.Combine(storage.RootPath, "crl", "tls-issuing.crl");
